...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<StoreCtx>(() => ({
    ...
    deleteQuestion: (id) => {
      ...
      if (/^[0-9a-f]{8}-/.test(id)) {
        supabase.from("questions").delete().eq("id", id).then(({ error }) => {
          if (error) console.warn("Delete question failed:", error.message);
        });
      }
    },
    bulkUpdateQuestionStatus: (ids, status) => {
      ...
      if (dbIds.length > 0) {
        supabase.from("questions").update({ status }).in("id", dbIds).then(({ error }) => {
          if (error) console.warn("Bulk status update failed:", error.message);
        });
      }
    },
